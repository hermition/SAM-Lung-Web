...
`case.json` 是追加式审计日志，包含用户名、输入图像摘要、模型信息、每次点击的像素坐标/前背景标签/UTC 时间、当时生效的全部 prompt、score 和对应 mask 路径。点击“保存结果”时还会记录 `saved_by`。撤销和清空也会记录，历史点击不会被删除。即使医生没有点击“保存结果”，输入和每次点击的 mask 也已经落盘。
