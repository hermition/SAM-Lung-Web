"""Backend helpers for the custom SAM1 interactive web UI."""
...
class SAMRunner:
    ...
    @torch.no_grad()
    def predict(self, predictor: SamPredictor, points: Sequence[Point]) -> Tuple[np.ndarray, float]:
        if not points:
...
class SessionStore:
    ...
    def __init__(self, runner: SAMRunner) -> None:
        self.runner = runner
